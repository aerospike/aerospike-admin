...
@test "build debian11 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="debian11" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-debian11"
...
@test "build debian12 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="debian12" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-debian12"
...
@test "build ubuntu20.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu20.04" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-ubuntu20.04"
...
@test "build ubuntu22.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu22.04" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-ubuntu22.04"
...
@test "build ubuntu24.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu24.04" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-ubuntu24.04"
...
@test "build redhat ubi9 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="redhat-ubi9" -v $TEMP_DIR:/tmp/output "asadmin-pkg-builder-redhat-ubi9"
