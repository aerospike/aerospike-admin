...
class SheetTest(unittest.TestCase):
    ...
    def test_sheet_project_boolean(self):
        test_sheet = Sheet(
            (Field('F', Projectors.Boolean('d', 'f')),),
            from_source='d')
    ...
    def test_sheet_project_float(self):
        test_sheet = Sheet(
            (Field('F', Projectors.Float('d', 'f')),),
            from_source='d')
        samples = [('1.25', 1.25, '1.25'),
                   (1.25, 1.25, '1.25'),
                   ('42', 42.0, '42.0'),
                   (42, 42.0, '42.0')]
    ...
    def test_sheet_project_func(self):
        test_sheet = Sheet(
            (Field('F', Projectors.Func(
                FieldType.boolean,
                lambda *values: 42 in values,
                Projectors.Number('d', 'f0'),
                Projectors.Number('d', 'f1'))),),
            from_source='d')
        samples = [('42', '58', True, 'True'),
                   (42, 58, True, 'True'),
                   (422, 588, False, 'False')]
    ...
    def test_sheet_no_entry(self):
        test_sheet = Sheet(
            (Field('F', Projectors.String('d', 'f'), hidden=False),),
            from_source=('d'))
        sources = dict(d=dict(n0={}))
        render = do_render(test_sheet, 'test', sources)
        value = render['groups'][0]['records'][0]['F']

        self.assertEquals(value['converted'], test_sheet.no_entry)
    ...
    def test_sheet_error_entry(self):
        ...
        self.assertEquals(value['converted'], test_sheet.error_entry)

    def test_sheet_flat_value(self):
        test_sheet = Sheet(
            (Field('F', Projectors.String('d', None)),),
            from_source=('d',)
        )
    ...
    def test_sheet_aggregation_no_entry(self):
        test_sheet = Sheet(
            (Field('F', Projectors.Number('d', 'f'), hidden=False,
                   aggregator=Aggregators.sum()),),
            from_source=('d',))
        sources = dict(d=dict(
            n0=dict(),
    ...
    def test_sheet_formatters(self):
        ...
        test_sheet = Sheet(
            (Field('F', Projectors.Number('d', 'f'),
                   formatters=(Formatters.red_alert(lambda edata: edata.value >= red),
                               Formatters.yellow_alert(lambda edata: edata.value >= yellow),
                               Formatters.green_alert(lambda edata: edata.value >= green))),),
            from_source='d')
        sources = dict(d=dict(
            n0=dict(f=red), n1=dict(f=yellow), n2=dict(f=green), n3=dict(f=none)))
        render = do_render(test_sheet, 'test', sources)
        records = render['groups'][0]['records']
