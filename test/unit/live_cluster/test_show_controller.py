...
class InfoControllerTest(asynctest.TestCase):
    ...
    async def test_do_release_success(self):
        ...
        with patch('lib.utils.util.filter_exceptions', return_value=build_data):
            with patch('lib.utils.util.callable') as callable_mock:
                ...
                self.cluster_mock.info_build.assert_called_once_with(nodes=["node1", "node2"])
                self.cluster_mock.info_release.assert_called_once_with(nodes=["node1", "node2"])
    ...
    async def test_do_network_enhanced_version_success(self):
        ...
        with patch('lib.utils.util.callable') as callable_mock:
            ...
            self.cluster_mock.info_statistics.assert_called_once_with(nodes=["node1", "node2"])
            self.cluster_mock.info.assert_called_once_with("cluster-name", nodes=["node1", "node2"])
            self.cluster_mock.info_build.assert_called_once_with(nodes=["node1", "node2"])
            self.cluster_mock.info_version.assert_called_once_with(nodes=["node1", "node2"])
