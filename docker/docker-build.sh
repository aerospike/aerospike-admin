...
ENVIRONMENT:
    JFROG_TOKEN             JFrog API token (or Identity Token) for downloading
                            packages from private JFrog repositories at build time.
                            When set, it is forwarded as a BuildKit secret and used
                            as a Bearer token by curl inside the Docker build.
                            Not needed when -u <local-dir> is used.
                            export JFROG_TOKEN=\$(jf config show | grep token | awk '{print \$2}')
...
function _emit_test_target() {
  ...
  echo "  secret     = JFROG_TOKEN != \"\" ? [\"id=jfrog_token,env=JFROG_TOKEN\"] : []"
  if [[ -n "${local_pkg}" ]]; then
...
function generate_bake() {
...

  {
    ...
    # JFROG_TOKEN is read from the environment at bake time.
    # When non-empty it is forwarded as a BuildKit secret so curl inside the
    # container can authenticate against private JFrog repositories.
    echo "variable \"JFROG_TOKEN\" { default = \"\" }"
    echo ""
