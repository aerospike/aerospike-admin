...
REQUIRED:
    -v, --version VERSION   asadm version, e.g. 5.0.0 or 5.0.0-rc2

                            A "rcN" pre-release tag is NOT part of any name: it
                            becomes the package iteration, so 5.0.0-rc2 resolves
                            the .deb 5.0.0-2<distro> and tags images 5.0.0-2.
                            A version with no rcN gets iteration 1.

OPTIONS:
    -i, --iteration N       Override the package iteration (default: from rcN,
                            else 1). Selects the .deb and the image tag.
...
TAGS PRODUCED:
  <tag> below is <version-without-rcN>-<iteration>, e.g. 5.0.0-rc2 -> 5.0.0-2
  and 5.0.0 -> 5.0.0-1.

  Test mode (loaded locally):
    <reg>/aerospike-asadm:<tag>-<arch>
    e.g. aerospike/aerospike-asadm:5.0.0-1-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-asadm:<tag>
    <reg>/aerospike-asadm:<tag>-<timestamp>   (when -T is set)
...
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-asadm:${IMAGE_TAG}-amd64"
...
function generate_bake() {
...

  {
    ...
    echo ""
    echo "variable \"VERSION\" { default = \"${IMAGE_TAG}\" }"
    echo ""
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asadm:${IMAGE_TAG}-${arch}")
      done
...
function resolve_packages() {
  local pkg_amd64="aerospike-asadm_${PKG_VERSION}-${ITERATION}ubuntu24.04_x86_64.deb"
  local pkg_arm64="aerospike-asadm_${PKG_VERSION}-${ITERATION}ubuntu24.04_aarch64.deb"
