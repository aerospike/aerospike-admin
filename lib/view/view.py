    ...
    class SummaryLicenseLine(SummaryLine):
        def __init__(
            ...
            try:
                ...
                s += f"Latest ({time_str}): {license_value}  Min: {file_size.size(license_dict['min']).strip()} Max: {file_size.size(license_dict['max']).strip()} Avg: {file_size.size(license_dict['avg']).strip()}"
