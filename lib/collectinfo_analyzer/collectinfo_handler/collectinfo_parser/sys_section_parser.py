...
def parse_sys_section(section_list, imap, parsed_map):
    ...
    for section in section_list:
        if section in parsed_map:
            ...
            util.type_check_basic_values(param_map)
...
def _parse_lsb_release_section(imap, parsed_map):
    ...
    sec_id_2 = "ID_26"
    raw_section_name_2, final_section_name_2, _ = util.get_section_name_from_id(
        sec_id_2
    )
...
def _modify_keys_in_iostat_section(iostatobj_list):
    for obj in iostatobj_list:
        util.change_key_name_in_map(obj, ["rkB/s"], "rk_b/s")
        util.change_key_name_in_map(obj, ["wkB/s"], "wk_b/s")
