...
def _parse_lsb_release_section(imap, parsed_map):
    ...
    if raw_section_name_1 not in imap and raw_section_name_2 not in imap:
        logger.warning(raw_section_name_1 + " and " +
                       raw_section_name_1 + " section not present.")
        return
    ...
    for section in lsb_section_names:
        ...
        if len(lsb_section_list) > 1:
            logger.warning(
                "More than one entries detected, There is a collision for this section: " + section)
...
def _parse_awsdata_section(imap, parsed_map):
    sec_id_1 = 'ID_70'
    raw_section_name_1, final_section_name_1, _ = get_section_name_from_id(
        sec_id_1)

    sec_id_2 = 'ID_85'
    raw_section_name_2, final_section_name_2, _ = get_section_name_from_id(
        sec_id_2)
    ...
    if raw_section_name_1 in imap and raw_section_name_2 in imap:
        ...
        logger.warning(raw_section_name_1 + " and " + raw_section_name_2 +
