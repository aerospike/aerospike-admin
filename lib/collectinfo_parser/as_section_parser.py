...
from .utils import (
    is_valid_section,
    get_section_name_from_id,
    is_bool,
    is_collision_allowed_for_section,
)
...
def _parse_build_version(imap, parsed_map):
    ...
    if not (
        is_valid_section(imap, raw_section_name_1, final_section_name_1)
...
def _get_nodes_from_network_info(imap):
    ...
    if not is_valid_section(
        ...
        raw_section_name,
        final_section_name,
        is_collision_allowed_for_section(sec_id),
    ):
        return
...
def _parse_nondefault_section(sec_id, nodes, imap, parsed_map):
    (
        ...
        final_section_name,
        parent_section_name,
    ) = get_section_name_from_id(sec_id)
...
def _parse_hist_dump_section(sec_id, nodes, imap, parsed_map):
    ...
    if not is_valid_section(
        ...
        raw_section_name,
        final_section_name,
        collision_allowed=is_collision_allowed_for_section(sec_id),
    ):
        return
