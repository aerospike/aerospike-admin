        ...
        if os.path.isdir(path):
            for file in self.get_files(True, path):
                ...
                if timestamp:
                    ...
                    error += ">>> Cannot add collectinfo file from asmonitor or any other log file other than collectinfo. Use the one from asadm. Ignoring " + file + " <<<\n"
            ...
            if timestamp:
                ...
                error += ">>>>> Missing Timestamp in file. Use the collectinfo using asadm <<<<< \n"
        else:
            error += ">>> " + path + " is incorrect path or not a aerospike file <<<"
