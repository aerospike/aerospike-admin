...
class ASSocket:
	...
	def _create_socket_for_addrinfo(self, addrinfo, tls_name=None, user=None, password=None, ssl_context=None):
		sock = None
		try:
			...
			if ssl_context:
				try:
					...
					print terminal.fg_red() + "TLS connection exception: " + str(tlse) + terminal.fg_clear()
					if sock:
						sock.close()
						sock = None
					return None
	...
	def _create_socket(self, host, port, tls_name=None, user=None, password=None, ssl_context=None):
		sock = None
		for addrinfo in socket.getaddrinfo(host, port, socket.AF_UNSPEC, socket.SOCK_STREAM):
			...
			try:
				sock = self._create_socket_for_addrinfo( addrinfo, tls_name, user, password, ssl_context=ssl_context)
				if sock:
					break
			except Exception:
				pass
	...
	def connect(self):
		try:
			self.sock = self._create_socket(self.ip, self.port, tls_name=self.node.tls_name, user=self.node.user, password=self.node.password, ssl_context=self.node.ssl_context)
			if not self.sock:
				return False
		except Exception:
			return False
	...
	def close(self, force=False):
		if self.sock:
			try:
				if not force and ( len(self.node.socket_pool[self.port]) < self.pool_size ):
